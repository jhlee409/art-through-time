...
큰 코드 파일을 분할할 때는 반드시 [공통 탭·전용 HTML 작성 규칙](작성규칙/새탭_template_작성규칙.md)의 코드 분할 기준을 확인합니다. 추출 파일은 원본 옆에 늘어놓지 않고 `원본 파일명(확장자 제외)/` 폴더 안에 저장하며, 기존 원본 경로는 필요한 경우 호환 진입점으로만 유지합니다.
