...
- 새 기법·용어를 추가할 때는 대표 이미지와 작품 설명을 `data/techniques/기법ID/` 아래 로컬 파일로 저장합니다. 예를 들어 `프레스테차(Prestezza)`는 틴토레토의 《노예의 기적》을 대표작으로 연결했습니다.
...
새 화가 데이터에는 반드시 `artistSummary: { "ko": [], "en": [] }` 기본 필드를 둡니다. 화면에서는 모든 화가의 대표작 위에 `화가 해설` 박스가 표시되며, 관리자는 이 박스의 `편집` 버튼으로 최대 6개 불릿 항목을 직접 쓰고 저장할 수 있습니다. 일반 사용자는 저장된 설명을 읽기만 할 수 있습니다.
