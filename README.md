...
화가 해설 오른쪽 위의 `편집`은 일반 텍스트를 직접 입력해 저장하는 기능입니다. `편집` 오른쪽의 `md 업로드`는 로컬 `.md` 또는 `.markdown` 파일을 브라우저에서 읽어 편집창 뒤쪽에 추가합니다. 블로그 글이나 사용자가 조사한 문장은 외부 웹을 읽지 않고 편집창에 직접 붙여 넣거나 md 파일로 불러옵니다. 일반 텍스트 입력, md 업로드, 스크립트 입력은 모두 OpenAI API를 사용하지 않는 입력 단계입니다.

`화가 및 작품`의 화가 이름 옆 `+`로 유튜브 주소를 추가하면 `스크립트 입력` 버튼이 나타납니다. 이 버튼에서 링크를 선택하고 유튜브의 `스크립트 표시` 내용을 붙여넣어 저장하거나 수정·삭제할 수 있습니다. 스크립트 저장은 OpenAI API를 사용하지 않으며, 저장 스크립트는 최대 80,000자입니다. 영상 파일이나 음원은 내려받지 않습니다.

`변환`은 OpenAI API를 사용하는 유일한 해설 버튼입니다. 현재 저장된 일반 텍스트·md 업로드 내용과 저장된 유튜브 스크립트를 전송한다는 확인창에 동의한 경우에만 문서형 해설로 정리합니다. 기존에 이미 변환된 내용은 삭제하지 않고, 사용자가 입력한 순서와 유튜브 링크 저장 순서를 유지합니다. 서로 다른 자료의 내용을 하나로 통합하거나 재배치하지 않고, 각 입력 블록 안에서만 문장을 읽기 좋게 다듬습니다. 변환 결과는 바로 저장하지 않고 편집창에 열어 사용자가 확인한 뒤 `저장`을 눌러 반영합니다. 이미지 URL이나 이미지 마크다운은 새 외부 이미지 의존을 만들지 않고, 작품명과 연도를 확인할 수 있을 때만 텍스트로 남깁니다. `.env`에 `OPENAI_API_KEY`를 설정하고 서버를 다시 시작해야 하며, 필요하면 `ART_ATLAS_SUMMARY_MODEL`로 모델을 바꿀 수 있습니다.
