...
화가 해설 오른쪽 위의 `편집`은 일반 텍스트를 직접 입력해 저장하는 기능입니다. 블로그 글이나 사용자가 조사한 문장은 외부 웹을 읽지 않고 이 편집창에 직접 붙여 넣어 저장합니다.
...
`변환`은 OpenAI API를 사용하는 유일한 해설 버튼입니다. 현재 저장된 해설과 저장된 유튜브 스크립트를 전송한다는 확인창에 동의한 경우에만 문서형 해설로 정리합니다. 기존에 이미 변환된 내용은 삭제하지 않고, 의미가 같은 내용은 합치며, 새 스크립트의 내용은 맥락에 맞는 위치에 보강합니다. 변환 결과는 바로 저장하지 않고 편집창에 열어 사용자가 확인한 뒤 `저장`을 눌러 반영합니다. 이미지 URL이나 이미지 마크다운은 새 외부 이미지 의존을 만들지 않고, 작품명과 연도를 확인할 수 있을 때만 텍스트로 남깁니다. `.env`에 `OPENAI_API_KEY`를 설정하고 서버를 다시 시작해야 하며, 필요하면 `ART_ATLAS_SUMMARY_MODEL`로 모델을 바꿀 수 있습니다.
