...
새 사조 설명 HTML은 해시 파일명을 만들지 않아도 됩니다. `신고전주의.html`처럼 사조 이름을 그대로 사용하고 `data/미술사조/index.json`에 활성 경로를 기록합니다. 전면 재구축이 완료되어 새 문서로 대체된 이전 해시 문서는 검증 후 색인과 파일에서 제거할 수 있습니다.
