...
const { canonicalArtworkFilename, refreshCatalog: refreshImageCatalog } = require('./tools/image-catalog');
...
process.once('uncaughtException', error => {
  if (error?.code === 'EADDRINUSE') {
    console.error(`Art Atlas is already running on http://localhost:${serverPort}`);
    process.exit(1);
  }
  throw error;
});
...
const serverPort = Number(process.env.ART_ATLAS_PORT || 4173);
...
function isJsonRequest(req, pathname) {
  ...
  return ['/api/auth/login','/api/artists','/api/artist-presentation','/api/artist-summary','/api/artist-summary-transform','/api/techniques','/api/movement-documents/refresh','/api/movement-section-links','/api/normalize-artist-works','/api/artwork-info'].includes(pathname);
}
...
const artistData = createArtistDataService({https,fs,path,URL,createHash,randomBytes,execFileAsync,ffmpegPath,root,dataDir,highResolutionDir,imageStagingDir,artistsFile,backupsDir,auditLogFile,adminEmail,artistImportedWorkLimit,highResolutionStoredLimit,sourceImageInputLimit,normalizeArtistsPayload,validateArtistsPayload,invalidArtworkThumbnail,writeUHangulArtistMap,syncPersonNameDictionary,canonicalArtworkFilename});
const { artistSearchCandidates, normalizeArtistWorks, artworkInfo, getJsonFast, api, similarityScore, getEntities, entityId, entityYear, entityLabel, koreanArtistNameOverrides, saveArtworkImageMutation, saveThumbnailBuffer, saveThumbnailFromLocalUpload, readArtistsFile, readArtistsIndex, readArtistDetail, writeArtistsFile, updateArtistPresentation, updateArtistSummary, highResolutionPathExists, thumbnailLocation, makePngUnderStorageLimit } = artistData;
const { transformArtistSummary } = require('./server-artist-research')();
const createContentService = require('./server-content');
const contentService = createContentService({fs,path,URL,createHash,randomBytes,execFileAsync,ffmpegPath,root,dataDir,highResolutionDir,imageStagingDir,techniquesFile,topicsFile,topicImageDir,movementSectionLinksFile,migrationAssetManifestFile,adminEmail,highResolutionStoredLimit,sourceImageInputLimit,jsonRequestBodyLimit,normalizeArtistsPayload,validateArtistsPayload,firebaseExport,invalidArtworkThumbnail,syncPersonNameDictionary,readAccessControl,readArtistsFile,writeArtistsFile,saveArtworkImageMutation,saveThumbnailBuffer,saveThumbnailFromLocalUpload,highResolutionPathExists,thumbnailLocation,makePngUnderStorageLimit,canonicalArtworkFilename,refreshImageCatalog});
const { migrationExport, safePath, techniqueLinks, comparisonTechniqueIds, readRequestBuffer, readJsonRequest, sendJsonBodyError, multipartForm, movementDocumentDir, movementDocumentName, movementDocumentSlot, movementDocumentRelative, readMovementDocuments, writeMovementDocuments, removeMovementDocument, refreshMovementDocumentLinks, saveMovementDocumentHtml, normalizeMovementCardPresentation, synchronizeMovementCountryTableArtistOrder, linkMovementDocumentArtists, injectUHangulDocumentIntegration, injectMovementArtworkMovementLabels, injectMovementCountryCardContexts, injectMovementPioneerContext, movementDocumentPioneerContextKey, injectMovementWikipediaHeading, injectMovementWikipediaTermLinks, injectMovementStickyTitle, injectMovementContentLayout, injectMovementHighResolutionViewer, saveLocalArtworkImage, saveNewLocalArtworkImage, saveTopicArtwork, replaceTopicArtworkImage, deleteTopicArtwork, readMovementSectionLinks, saveMovementSectionLinks, applyCors } = contentService;
http.createServer(async (req,res) => { const url=new URL(req.url,`http://${req.headers.host}`); applyCors(req,res); if(req.method==='OPTIONS') { res.writeHead(204); return res.end(); } if (!enforceJsonRequestLimit(req,res,url.pathname)) return;
  ...
  if (req.method==='PUT' && url.pathname==='/api/artist-summary') { try { const result=await updateArtistSummary(JSON.parse(await readJsonRequest(req) || '{}'),session.email); res.writeHead(200,{'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store'}); res.end(JSON.stringify({ok:true,...result})); } catch(error) { console.error('Artist summary save failed:',error?.stack || error?.message || error); sendJsonBodyError(res,error,error?.statusCode || 422); } return; }
  if (req.method==='POST' && url.pathname==='/api/artist-summary-transform') { try { const body=JSON.parse(await readJsonRequest(req) || '{}'); if(body.consent!==true) throw new Error('OpenAI API 전송에 대한 명시적 동의가 필요합니다.'); const artist=await readArtistDetail(String(body.artistId || '')); if(!artist) throw new Error('Artist not found'); const result=await transformArtistSummary(artist); res.writeHead(200,{'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store'}); res.end(JSON.stringify({ok:true,...result})); } catch(error) { console.error('Artist summary transform failed:',error?.stack || error?.message || error); sendJsonBodyError(res,error); } return; }
  ...
  if (req.method==='POST' && url.pathname==='/api/local-thumbnail-image') { try { const form=multipartForm(await readRequestBuffer(req,sourceImageInputLimit + 1024*1024),req.headers['content-type']), result=await saveNewLocalArtworkImage(form,session.email); res.writeHead(200,{'Content-Type':'application/json','Cache-Control':'no-store'}); return res.end(JSON.stringify({ok:true,...result})); } catch(error) { res.writeHead(422,{'Content-Type':'application/json','Cache-Control':'no-store'}); return res.end(JSON.stringify({ok:false,thumbnail:'',verified:false,error:error.message})); } }
