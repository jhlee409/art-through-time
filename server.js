...
function isJsonRequest(req, pathname) {
  ...
  return ['/api/auth/login','/api/artists','/api/artist-presentation','/api/techniques','/api/movement-documents/refresh','/api/movement-section-links','/api/normalize-artist-works','/api/artwork-info'].includes(pathname);
}
...
const { artistSearchCandidates, normalizeArtistWorks, artworkInfo, getJsonFast, api, similarityScore, getEntities, entityId, entityYear, entityLabel, koreanArtistNameOverrides, saveThumbnailBuffer, saveThumbnailFromLocalUpload, readArtistsFile, readArtistsIndex, readArtistDetail, writeArtistsFile, updateArtistPresentation, highResolutionPathExists } = artistData;
