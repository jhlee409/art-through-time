...
window.name='artThroughTimeTechniques';
...
function setUHangulMode(mode){uHangulMode=['original','uhangul'].includes(mode)?mode:'korean';sessionStorage.setItem(uHangulModeStorageKey,uHangulMode);syncPageNavLinks();window.dispatchEvent(new CustomEvent('uhangulmodechange',{detail:{mode:uHangulMode}}));}
...
const techniqueLinkEmphasisField=()=>`<label class="link-emphasis-field"><input type="checkbox" data-link-emphasis><span>${language==='ko'?'강조':'Emphasize'}</span></label>`;
...
function setupTechniqueLinkEntry(technique) {
  ...
  if(entry)entry.onsubmit=async event=>{event.preventDefault();const input=entry.querySelector('input[type="url"]');let url;try{url=new URL(input.value.trim());if(!['http:','https:'].includes(url.protocol))throw new Error();}catch(_){input.setCustomValidity(language==='ko'?'http 또는 https 주소를 입력하세요.':'Enter an http or https URL.');input.reportValidity();input.setCustomValidity('');return;}const emphasized=entry.querySelector('[data-link-emphasis]')?.checked===true;if(await saveTechniqueLinks(technique,[...techniqueLinks(technique),{url:url.href,...(emphasized?{emphasized:true}:{})}]))renderTechnique(technique);};
}
function renderTechnique(technique){
  ...
  const savedLinks=techniqueLinks(technique),addLinkLabel=language==='ko'?'자료 추가':'Add resource',linkInputLabel=language==='ko'?'자료 URL을 입력하세요':'Enter a resource URL',confirmLinkLabel=language==='ko'?'확인':'Add',linkButtons=savedLinks.map((link,index)=>`<button class="technique-link-button${link.emphasized===true?' link-emphasized':''}" type="button" data-technique-link-index="${index}" title="${esc(link.url)}" aria-label="${esc(`${index+1}. ${link.url}`)}">${index+1}</button>`).join(''),linkAdd=token?`<button class="technique-link-add" type="button" title="${esc(addLinkLabel)}" aria-label="${esc(addLinkLabel)}">+</button>`:'',linkControls=`<span class="technique-link-controls">${linkButtons}</span>`,linkEntry=token?`<form class="technique-link-entry hidden"><input type="url" inputmode="url" placeholder="https://" aria-label="${esc(linkInputLabel)}" required>${techniqueLinkEmphasisField()}<button type="submit">${esc(confirmLinkLabel)}</button></form>`:'';
...
techniqueLogoutButton.onclick=async()=>{if(typeof window.artThroughTimeLogoutAll==='function')return window.artThroughTimeLogoutAll();const saved=JSON.parse(sessionStorage.getItem('art-atlas-access-session-v1')||'null');try{await fetch('/api/auth/logout',{method:'POST',headers:saved?.token?{Authorization:`Bearer ${saved.token}`}:{}});}catch(_){}try{sessionStorage.removeItem('art-atlas-access-session-v1');localStorage.setItem('art-atlas-logout-signal',String(Date.now()));}catch(_){}location.assign('index.html?login=1');};
