...
function renderTechnique(technique){
  ...
  techniqueList.innerHTML=sortedTechniques().map(item=>{const canDelete=Boolean(token);return `<div class="technique-list-row"><button class="technique-item ${item.id===technique.id?'active':''}${canDelete?' has-delete':''}" data-technique="${esc(item.id)}"${canDelete?` data-delete-technique="${esc(item.id)}" title="${esc(copy[language].delete)}"`:''}><span class="technique-name"${techniqueListLanguage==='en'?' data-uh-ignore="true"':''}>${esc(text(item.name,techniqueListLanguage))}</span><small class="technique-year">${esc(item.introduced)}</small></button></div>`;}).join('');
...
techniqueLanguageToggle?.addEventListener('click',()=>setTimeout(renderTechniqueSearchResults,0));
