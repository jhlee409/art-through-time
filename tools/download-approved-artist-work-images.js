...
function approvedDownloadUrl(item) {
  ...
  if (url.protocol !== 'https:' || !approvedHost) throw new Error(`${item.artistId}: unapproved image host ${url.hostname}`);
  return url;
}
...
async function fetchImage(url, artistId) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    response = await fetch(url, {
      headers: {'User-Agent': 'ArtThroughTime/0.1 (one-time user-approved educational artist image download)'},
      signal: AbortSignal.timeout(30000)
    });
    if (response.status !== 429) break;
    const retryAfter = Number(response.headers.get('retry-after') || 0);
    const waitMs = Math.min(30000, Math.max(retryAfter * 1000, 6000 * (attempt + 1)));
...
function updateWorkImage(work, item, localPath) {
  ...
  work.source = `Wikimedia Commons: ${item.selected.pageUrl}`;
  work.thumbnailValidation = 2;
  work.thumbnailCacheKey = item.downloadedAt || new Date().toISOString();
  work.migration = {
    schema: 1,
    image: {
      status: 'ready',
      ...
      sourceUrl: item.selected.pageUrl,
      sourceUrls: [...new Set([item.selected.pageUrl, item.selected.downloadUrl].filter(Boolean))],
...
async function main() {
  ...
  const persist = () => {
    manifest.counts = {
      ...
      skippedNoSearchTokens: manifest.counts?.skippedNoSearchTokens || 0
    };
  ...
  for (const item of approved) {
    try {
      ...
      if (repWork) {
        ...
        repWork.institution = 'Wikimedia Commons';
      }
      ...
      if (/\b429\b/.test(error.message)) {
        ...
        if (consecutiveRateLimits >= 8) {
          console.error('Stopping early after 8 consecutive Wikimedia rate-limit responses.');
          break;
        }
      } else {
        consecutiveRateLimits = 0;
