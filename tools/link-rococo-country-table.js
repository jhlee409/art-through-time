...
const people = {
  '와토':['artist-Q155151','Antoine Watteau','앙투안 와토'], '부셰':['artist-Q313122','François Boucher','프랑수아 부셰'], '프라고나르':['artist-Q313898','Jean-Honoré Fragonard','장오노레 프라고나르'], '비제 르 브룅':['artist-Q213163','Élisabeth Louise Vigée Le Brun','엘리자베스 루이 비제 르 브룅'],
  '티에폴로':['artist-Q162048','Giovanni Battista Tiepolo','조반니 바티스타 티에폴로'], '카날레토':['artist-Q161866','Canaletto','카날레토'], '게인즈버러':['artist-Q180117','Thomas Gainsborough','토머스 게인즈버러'], '호가스':['artist-Q189621','William Hogarth','윌리엄 호가스'], '샤르댕':['artist-Q312309','Jean-Baptiste-Siméon Chardin','장바티스트 시메옹 샤르댕'], '로살바 카리에라':['artist-Q235547','Rosalba Carriera','로살바 카리에라']
};
const link = name => { const [id,en,ko] = people[name]; return `<a class="art-atlas-artist-link" href="../../index.html?artist=${id}" target="_blank" rel="noopener" data-artist-id="${id}" data-uh-original="${en}" data-uh-korean="${ko}" data-uh-display-korean="${name}" title="${ko} 연표로 이동">${name}</a>`; };
...
for (const name of ['와토','부셰','프라고나르','티에폴로','카날레토','게인즈버러','호가스']) table = table.replace(name, link(name));
...
if (!html.includes('alt="와토, 키테라섬으로의 순례"')) html = html.slice(0, gridStart + grid.length) + cards + html.slice(gridStart + grid.length);
