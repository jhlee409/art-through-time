...
const standardHeaders=['국가·지역·세부 사조','지역적 특징','대표 화가·제작자'];
...
function normalizeTable(table) {
  ...
  if (headers.length !== 3) return {table,reason:`${headers.length}컬럼`};
...
function normalizeDocument(source) {
  ...
  if (!new RegExp(`id=["']${styleId}["']`,'i').test(next)) next=/<\/head>/i.test(next) ? next.replace(/<\/head>/i,`${style}</head>`) : `${style}\n${next}`;
