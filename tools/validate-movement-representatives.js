...
function validateLearningMapDocument(parent, artistMap) {
  ...
  const artistGuide = attr(rootTag, 'data-art-atlas-document-model') === 'artist-guide';
  const nodes = learningMap.movements?.[parent.id]?.nodes || [];
  assert(nodes.length === 5, `${parent.id}: expected five learning nodes`);
  nodes.forEach(node => {
    const rowPattern = artistGuide
      ? new RegExp(`<tr\\b(?=[^>]*${attrs.learningNodeId}=["']${node.id}["'])(?=[^>]*${attrs.artistId}=["']${node.artist.id}["'])(?=[^>]*${attrs.workId}=["']${node.work.id}["'])[^>]*>`, 'i')
      : new RegExp(`<tr\\b(?=[^>]*${attrs.learningNodeId}=["']${node.id}["'])[^>]*>`, 'i');
    ...
    if (!artistGuide) assert(groupPattern.test(html), `${node.id}: learning card group is missing`);
