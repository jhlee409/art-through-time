...
- 새로 만들거나 전면 재구축하는 활성 사조 문서는 해시 파일명을 요구하지 않는다. `신고전주의.html`처럼 사람이 식별할 수 있는 한국어 또는 영어 사조 이름을 파일명으로 사용하고 `data/미술사조/index.json`에 경로를 등록한다. 같은 사조의 두 번째 문서 슬롯이 필요하면 이름 뒤에 `-2`를 붙인다.
