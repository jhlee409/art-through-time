...
`data/미술사조/*.html`을 만들거나 수정할 때 적용한다. 재사용할 규칙·예외·레이아웃·검증 절차가 바뀌면 같은 작업에서 이 문서를 갱신한다. 사조 설명 문서, 국가 전개 표, 대표작 카드, 사조 HTML 제공 경로를 수정하기 전에는 README의 `작업 전 필수 작성규칙` 절차와 이 문서를 먼저 확인한다. 기존 문서의 분류·길잡이·표·심화 카드를 함께 새 구조로 바꾸는 작업에서는 [사조 HTML 변환 규칙](사조html_변환규칙.md)도 반드시 함께 적용한다.

## 구조와 이름

- 새로 만들거나 전면 재구축하는 사조 문서는 `data-art-atlas-document-model="artist-guide"`를 루트에 기록한다. 이 모델은 국가·지역 기반 작가 분류를 사용하지 않으며 `정의`, `태동`, `확립`, `비판과 전환`, `공통 특징`, `대표 미술가`, `화파`, `대표 작품 카드` 순서로 초심자의 이해를 단계적으로 만든다.
- 대표 미술가 표는 `학습 단계`, `미술가·분야`, `주요 활동 국가`, `미술가 고유의 특징`, `사조 특징을 가장 잘 보여 주는 작품·연도`를 기본 컬럼으로 쓴다. 활동 국가는 정보 컬럼일 뿐 범주·행·카드 묶음의 ID가 아니다.
- 대표 미술가는 `초심자 핵심`을 먼저 두고 더 깊은 맥락은 `중급 확장`으로 추가한다. 표 행과 카드에는 `data-art-atlas-artist-tier="beginner|intermediate"`, `data-artist-id`, `data-work-id`를 동일하게 기록한다.
- 명백하게 알려진 화파, 작업실 계보, 선언 집단이 있을 때만 별도 화파 표를 둔다. 단순히 같은 국가에서 활동했다는 이유로 화파를 만들지 않는다.
- 기존 국가 전개형 문서는 해당 사조가 전면 재구축되기 전까지 버전 1 호환 문서로 유지한다. `artist-guide`로 전환한 문서에는 `#countries`, `developmentId`, `categoryId`, `countryIds` 기반의 국가 전개 행과 카드 묶음을 남기지 않는다.
...
- 정본 36개 색인에서 제외했지만 아직 내용 출처로 필요한 기존 하위 사조·중복 문서는 `data/미술사조/legacy-index.json`에 소유 부모·범주·용도를 기록한다. 전면 재구축이 완료되어 더 이상 원문 보존이 필요 없는 교체 문서는 검증 후 레거시 색인과 파일에서 제거할 수 있다. 실제 폴더의 HTML 중 활성 36개를 제외하고 남겨 둔 모든 파일은 레거시 색인에 정확히 한 번 있어야 하며, 활성 문서와 레거시 문서가 중복 등록되어서는 안 된다.
- 새로 만들거나 전면 재구축하는 활성 사조 문서는 반드시 `신고전주의.html`처럼 사람이 식별할 수 있는 한국어 또는 영어 사조 이름을 파일명으로 사용하고 `data/미술사조/index.json`에서 그 경로를 직접 연결한다. 같은 사조의 두 번째 문서 슬롯이 필요하면 이름 뒤에 `-2`를 붙인다. 교체가 완료된 이전 해시 HTML은 활성 폴더와 색인에서 제거하며, 해시 파일 탐색·리다이렉트·대체 경로 fallback으로 새 이름 문서에 연결하지 않는다.
...
- 태동·확립·비판과 전환은 양식의 호불호만으로 설명하지 않는다. 각 구간은 기본적으로 두 작품을 좌우에 놓는 대립형 비교 도판으로 구성한다. 태동은 `극복하려 한 이전 사조의 전형 ↔ 새 사조 초기의 대비되는 작품`, 확립은 `서로 시간 차이가 있어 변화 과정을 보여 주는 대표작 2점`, 비판과 전환은 `후발 사조가 비판한 특징이 선명한 작품 ↔ 대안 사조의 전형적인 작품`을 한 쌍으로 삼는다. 각 쌍의 설명은 화면에서 직접 비교할 조형 차이와 함께 사회·정치·경제·철학·사상·후원 제도가 변화에 어떻게 연결되는지 서술한다. 이전 사조의 작품을 사회적 부패의 직접 증거로 단정하지 않고, 당대 비평과 관객이 그 형식에 어떤 정치적 의미를 부여했는지 구분한다.
...
- 학습 길잡이 설명의 블록 스타일은 `.movement-learning-guide-categories li > span`처럼 설명을 담은 직계 자식에만 적용한다. uHangul이 이름을 감싸는 내부 `span`까지 `display:block`이나 줄바꿈 규칙을 받게 하지 않는다.

## 국가 전개와 카드

아래 국가 전개 규칙은 아직 `artist-guide`로 전환하지 않은 기존 문서의 호환 규칙이다. 새 문서와 전면 재구축 문서에는 적용하지 않는다.

- 신고전주의는 `artist-guide` 전환의 첫 적용 문서다. 다비드·카노바·앵그르는 `초심자 핵심`, 멩스·카우프만은 `중급 확장`으로 두며, 활동 국가는 분류가 아닌 정보로만 표시한다. 이 구조는 신고전주의 전용 예외가 아니라 이후 전면 재구축 문서에 적용할 새 기본 모델이다.
...
- 기존 국가 전개 문서에서는 대표 화가 카드를 묶음의 첫 카드로 고정한다. 더 볼 화가 카드만 같은 묶음 안에서 드래그할 수 있고, 그 순서는 표의 `data-art-atlas-further-artists` 셀에 동기화한다.
...
- `data-art-atlas-learning-node-id`가 있는 학습 지도 카드 묶음은 정식 국가별 범주 보정 대상이 아니다. 기준점마다 정한 고유 분류 제목과 설명을 원문 그대로 유지하며, 서버의 일반 국가·범주 제목 주입으로 이를 덮어쓰지 않는다.
- 학습 지도 카드 묶음이 같은 `categoryId`를 공유하더라도, CSS 의사 요소로 범주 공통 제목·형성 맥락·핵심 특징을 대체해 반복 표시하지 않는다. 각 `learningNodeId`의 원문 제목과 설명을 렌더링한다.
- 초심자 학습 길잡이는 기본적으로 정식 `categoryId`의 핵심 범주를 비교한다. 단 학습 지도가 정의된 사조는 `data/art-movement-learning-map.json`의 기준점·주요 변주·비교 축을 표시하며, 정식 범주의 국가명으로 이를 다시 치환하지 않는다.
- 학습 지도 사조의 초심자 길잡이는 기준점·변주 설명을 한 열 전체 폭으로 표시한다. 긴 한 문장을 여러 개의 좁은 열에 임의로 나누지 않으며, 다른 사조의 일반 길잡이 2열 구성에는 영향을 주지 않는다.
- 학습 지도 카드와 길잡이의 형성 맥락·핵심 특징은 문장 내부의 단어와 조사가 분리되지 않게 `word-break: keep-all`을 쓰고, 데스크톱에서는 한 문장을 한 줄로 표시한다.
...
- 기존 국가 전개형 버전 1 카드 드래그는 같은 `developmentId`의 `cardRole="further"` 카드끼리만 허용하고 표의 더 볼 화가 링크 순서를 같은 저장 트랜잭션에서 바꾼다. 대표 카드는 항상 첫 위치에 고정하며 다른 전개·범주로 화가를 옮길 때는 카드 드래그를 사용하지 않는다.
- `artist-guide` 문서의 하단 대표 화가 카드는 같은 `data-art-atlas-artist-tier` 그리드 안에서 모두 드래그해 순서를 바꿀 수 있다. `초심자 핵심`과 `중급 확장` 사이로 카드를 옮겨 등급을 바꾸는 것은 허용하지 않는다. 저장할 때 대표 화가 표의 같은 화가 행을 카드 DOM 순서에 맞춰 한 트랜잭션에서 재정렬하며, 저장된 HTML의 순서를 다음에 문서를 열 때의 기본 순서로 사용한다.
...
- 사조명 바로 아래 소개문과 각 절의 기본 설명문은 별도의 비교 패널·도판·그리드가 필요한 경우가 아니면 문서의 좌우 안쪽 여백 사이 전체 폭을 사용한다. 관습적인 `max-width`로 오른쪽 본문 공간을 비워 두지 않는다.
...
- 사조 설명 문서의 대표작 카드 이미지를 더블클릭하면 현재 카드 크기와 무관하게 원본 비율을 유지하면서 가로 또는 세로 중 먼저 닿는 쪽이 현재 화면의 `90%`가 되도록 확대한다. 확대 이미지에는 가능한 경우 화가 작품 데이터의 고해상도 로컬 파일을 사용한다. 확대 화면은 마우스 휠과 돋보기 버튼으로 확대·축소하고, 확대된 이미지는 포인터 드래그로 이동하며, 초기화 버튼으로 화면 맞춤 상태로 돌아간다. 확대 이미지나 배경을 다시 더블클릭하거나 닫기 버튼·`Escape` 키를 누르면 원래 문서 화면으로 복귀한다.
