...
- 관리자 예외 — 신고전주의 시범: 국가 전개 표와 심화 대표작 카드의 **표시 분류**는 국가·도시명이 아니라 다비드(공적 역사화·시민 윤리), 카노바(이상적 형식·인체), 멩스(이론·라파엘로적 질서), 앵그르(선·표면·이상화)의 네 조형 기준점과 카우프만(국제 역사화의 주요 변주)으로 쓴다. 프랑스·로마 등은 그 지향이 형성·활동한 맥락으로만 병기한다. 표는 이 학습 지도에 한해 `조형 기준점·주요 변주`, `핵심 특징`, `대표 화가`의 3컬럼만 두며, 빈 ‘주요 변주 화가’ 공통 컬럼을 만들지 않는다. 다섯 항목은 각각 독립된 표 행과 대표작 카드 묶음을 가지되, `data/art-movement-learning-map.json`의 `learningNodeId`가 화가·작품·학습 역할의 정본이다. 각 항목은 기존 정식 `categoryId` 하나만 참조하며 새 정식 범주·국가별 사조 막대 연결을 만들지 않는다. 심화 대표작 카드 묶음은 넓은 화면에서 3열, 중간 화면에서 2열, 좁은 화면에서 1열로 배치하되 카드 내부 내용과 이미지 영역은 바꾸지 않는다. `신고전주의의 주요 조형 지향과 화가` 안내의 네 기준점 컬럼은 가능한 화면 폭에서 같은 행을 공유하고 좁은 화면에서만 줄바꿈하며, 카우프만은 전체 폭 주요 변주로 별도 표시한다. 나폴레옹 시대의 공적 이미지는 별도 비교 축으로 두고 다비드·앵그르의 로컬 작품 카드로 영웅화·의례·권위의 차이를 비교한다. 이 지도는 배타적인 사조 분류가 아니라 작품 비교용 좌표이며, 이 예외는 신고전주의 문서에만 적용한다.

- 신고전주의의 3열 학습 카드에서는 분류 제목·형성 맥락·카드가 각 열 내부 폭에만 배치된다. 공통 카드 묶음 제목의 전체 폭 규칙을 이 예외에 적용하지 않는다.
