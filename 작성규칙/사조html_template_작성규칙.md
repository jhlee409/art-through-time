...
`data/미술사조/*.html`을 만들거나 수정할 때 적용한다. 재사용할 규칙·예외·레이아웃·검증 절차가 바뀌면 같은 작업에서 이 문서를 갱신한다. 사조 설명 문서, 국가 전개 표, 대표작 카드, 사조 HTML 제공 경로를 수정하기 전에는 README의 `작업 전 필수 작성규칙` 절차와 이 문서를 먼저 확인한다.
...
- 사조 설명의 일반 본문과 표는 창의 전체 가로 폭에서 공통 좌우 여백(`clamp(20px, 3vw, 48px)`)만 두고 표시한다. 본문·표에 임의의 최대 폭을 다시 두지 않는다. 대표작 카드 묶음과 카드 이미지·카드 내부 콘텐츠의 크기·레이아웃은 이 규칙의 대상이 아니다.
- 공통 활자 단계는 문서 제목(`h1`), 큰 제목(`h2`), 중간 제목(`h3`), 작은 제목(`h4`), 강조 본문 1(구역 첫 설명문), 본문 2(후속 설명문) 순으로 사용한다. 카드 내부 제목·메타·설명은 대표작 카드 전용 규칙을 유지하며 일반 본문 단계로 덮어쓰지 않는다.
...
- 활성 사조 설명의 심화 보강에는 `data-art-atlas-country-development-guide="1"`인 `지역 전개를 한눈에 비교하기` 문단을 둔다. 이 문단은 국가별 발전을 단순 나열하지 않고, 지역마다 달라진 정치·제도·관객·매체 조건과 작품에서 비교할 시각적 기준을 초심자 언어로 함께 설명한다. 표의 국가·범주·화가·기간 ID와 대표작 카드의 내용은 이 보충 문단 때문에 바꾸지 않는다.
...
- 카드 상단은 `화가명, 《작품명》 · 사조 · 지역`을 왼쪽에, 제작 연도를 같은 행의 오른쪽에 둔다. 긴 작품명은 왼쪽 영역에서 줄바꿈하되 연도는 줄바꿈하지 않는다.
- 카드의 강조 설명 레이블은 반드시 `선정 이유:` 또는 `더 볼 이유:`처럼 콜론까지 굵게 표시한다.
...
- 심화 대표작 카드의 검은 이미지 영역 크기는 유지한다. 그 안의 작품 이미지는 가로·세로를 각각 영역의 90%까지 쓰고 `object-fit: contain`으로 원본 비율과 전체 화면을 보존해, 약간의 여백만 남기고 크게 보이게 한다.
